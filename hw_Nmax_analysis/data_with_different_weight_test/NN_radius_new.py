...
def NN_all(input_path,output_path,data_num,monitor,min_delta,patience,epochs,input_dim,output_dim,interpol_count,max_nmax_fit):
    #max_nmax_fit = 14
...
#
    ...
    for loop in range(0,200):
        ...
        raw_data_new[loop,0] = np.max(raw_data[np.where(raw_data[:,1]==loop+4)])
    ...
    plt.title("radius(converge)="+str(radius_converge))
    
    plt.ylabel("lg(radius(converge)-radius(infinte))")
...
for max_nmax_fit in range(18,21,2):
    ...
    with open(nuclei+'/'+target_option+'/radius-nmax4-'+str(max_nmax_fit)+'/'+'radius_NN_info.txt','a') as f_3:
        #f_3.read()
        f_3.write('#######################################################'+'\n')
        f_3.write('# loop   radius_energy        loss           val_loss'+'\n')
    for loop_all in range(run_times_start-1,run_times_end):
        ...
        with open(nuclei+'/'+target_option+'/radius-nmax4-'+str(max_nmax_fit)+'/'+'radius_NN_info.txt','a') as f_3:
            ...
            f_3.write('{:>-12.10f}'.format(loss_all[loop_all])+'     ')
            f_3.write('{:>-12.10f}'.format(val_loss_all[loop_all])+'\n')
